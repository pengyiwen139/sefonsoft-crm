...
//        contactInfoQueryDTO.setEmployeeId(loginUserDTO.getUserId());
        ...
        List<ContactInfoQueryDTO> contactInfoQuery = contactInfoService.getCondition(contactInfoQueryDTO);
        int totalContactInfoQuery = contactInfoService.findCondition(contactInfoQueryDTO);
    ...
    public Response selectOne(@ApiParam(required = true, value = "表主键id") @PathVariable Long id) {
        if (ObjTool.isNotNull(id)) {
            ...
            ContactInfo contactInfo = contactInfoService.queryById(id);
    ...
    public Response contactList(@ApiParam(required = true, value = "表主键id") String customerId) {
        if (ObjTool.isNotNull(customerId)) {
            List<ContactInfo> contactInfo = contactInfoService.queryContactInfoByCustomerId(customerId);
    ...
    public Response insert(@Valid @RequestBody ContactInfo contactInfo, BindingResult result,
        ...
        return contactInfoService.insert(contactInfo) ? success() : failure(INSERT_ERROR);
    }
    ...
    public Response update(@Valid @RequestBody ContactInfo contactInfo, BindingResult result) {
        ...
        return contactInfoService.update(contactInfos) > 0 ? success() : failure(UPDATE_ERROR);
    }
    ...
    public Response delete(@ApiParam(required = true, value = "表主键id") @PathVariable Long id) {
        ...
        return contactInfoService.deleteById(id) ? success() : failure(DELETE_ERROR);
    }
    ...
    @RequestMapping(value = "/contact/delAll", method = RequestMethod.DELETE)
    ...
    public Response deleteIds(String ids) {
        ...
        return contactInfoService.deleteByIds(idArray)? success() : failure(DELETE_ERROR);
    }
